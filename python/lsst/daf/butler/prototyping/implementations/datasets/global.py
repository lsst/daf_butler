...
def _makeDynamicTableName(datasetType: DatasetType) -> str:
    return f"dataset_global_{datasetType.dimensions.encode().hex()}"
...
def _makeDynamicTableSpec(datasetType: DatasetType) -> TableSpec:
    ...
    # DatasetTypes with global uniqueness have a constraint on just dataset
    # type + data ID.  We only bother including the required part of the data
