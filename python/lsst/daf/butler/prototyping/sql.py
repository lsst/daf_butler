...
class Database(ABC):
    ...
    @abstractmethod
    def sync(self, table: sqlalchemy.schema.Table, key: str, row: dict,
...
class SqlGeneralRecordStorage(GeneralRecordStorage):
    ...
    def syncRun(self, name: str) -> Run:
        collectionRow = self._db.sync(
            self._tables.collection, "name",
            {"origin": self._db.origin, "name": name, "type": CollectionType.RUN},
        )
        runRow = self._db.sync(self._tables.run, "collection_id",
                               {"collection_id": collectionRow["id"], "origin": self._db.origin})
        return Run(**runRow)

    def findRun(self, name: str) -> Optional[Run]:
        sql = self._tables.run.select().where(self._tables.run.columns.name == name)
        return Run(**self._db.connection.execute(sql).fetchone())
