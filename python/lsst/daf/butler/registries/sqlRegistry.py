...
class SqlRegistry(Registry):
    ...
    def getDatasetType(self, name):
        ...
        # Get Dinmensions (if any) from DatasetTypeDimensions table
    ...
    def addDimensionEntry(self, dimension, dataId=None, entry=None, **kwds):
        ...
        if entry is not None:
            dataId.entries[dataId.dimension].update(entry)
        table = self._schema.tables[dataId.dimension.name]
        if table is None:
            raise TypeError(f"Dimension '{dataId.dimension.name}' has no table.")
        try:
            self._connection.execute(table.insert().values(**dataId.fields(dataId.dimension, region=False)))
    ...
    def _queryMetadata(self, element, dataId, columns):
        ...
        row = self._connection.execute(
            select(cols)
            .where(
                and_(table.c[name] == dataId[name] for name, value in dataId.items())
            )
        ).fetchone()
        if row is None:
