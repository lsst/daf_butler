...
class RemoteButlerRegistry(Registry):
    ...
    def findDataset(
        ...
        **kwargs: Any,
        ...
        # raise an exception in registry.findDataset.  Not sure why, but
        # someone went through the trouble of writing a fairly lengthy unit
        # test (RegistryTests.testComponentLookups) only for the purpose of
        # verifying this behavior with a note that "Components can no longer be
        # found by registry", implying that it used to work but was
        # intentionally removed for reasons.
