...
__all__ = ("S3FitsExposureFormatter", )
...
class S3FitsExposureFormatter(Formatter):
    ...
    def readFull(self, fileDescriptor, parameters=None):
        ...
        # this is just an absolute excercise in laziness,
        # storing all downloads under the same name overwrites them, but they
        # look to be in memory so I guess its fine for learning purposes.
        fileDescriptor.storageClass.validateParameters(parameters)
        savepath = "/home/dinob/uni/lsstspark/simple_repo/s3_repo/image.fits"
        s3client = boto3.client('s3')
        s3client.download_file(fileDescriptor.location._bucket,
                               fileDescriptor.location.path,
                               savepath)

        return fileDescriptor.storageClass.pytype(savepath, **parameters)
