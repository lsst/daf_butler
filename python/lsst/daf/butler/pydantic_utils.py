...
class DeferredValidation(Generic[_T]):
    ...
    schema of the wrapper will be identical to the JSON schema of the wrapped
    type, and Pydantic serialization will work regardless of whether the
    wrapper instance was initialized with the raw type or the wrapped type.
    Pydantic validation of the wrapper will effectively do nothing, however;
    instead, the `validated` method must be called to return a fully-validated
    instance of the wrapped type, which is then cached within the wrapper for
    subsequent calls to `validated`.
