...
class DataId(Mapping):
    ...
    They keys of a `DataId` correspond to the "link" columns of one or more
    `Dimension`\s, while values identify particular rows in the tables or view
    for those `Dimension`\s.  In addition to implementing the (immutable)
    `collections.abc.Mapping` API, `DataId`\s have additional attributes to
    hold additional metadata, regions, and definitions for those `Dimension`\s.
    They are also hashable, and hence can be used as keys in dictionaries.
    ...
    entries : `dict`, optional
        ...
        column values as innter dictionary values.
    extra : `dict`, optional
    ...
    def __new__(cls, dataId=None, *, dimensions=None, dimension=None, universe=None, region=None,
        ...
        if dimensions is None:
            ...
            # 'dimensions' is provided, but 'dimension' is not; look to see
            # if there is any single dimension whose dependency graph is
            # exactly that set of dimensions.
            if dimension is None:
                for candidate in dimensions:
                    if candidate.graph() == dimensions:
                        dimension = candidate
                        break
            elif dimension.graph() != dimensions:
            ...
            for linkName, linkValue in items:
                try:
                    ...
                    if dimension is not None:
                        self.entries[self.dimension][linkName] = linkValue
                    else:
                        raise
    ...
    @property
    def dimension(self):
        """The unique leaf dimension this `DataId` identifies (`Dimension` or
        ``None``).

        DataId objects are in general associated with multiple dimensions;
        this attribute is only set if one dimension has all of the others in
        its required dependencies.
        """
    ...
    def __hash__(self):
        return hash(tuple(self._links.items()))
    ...
    def fields(self, element, region=True, metadata=True):
        ...
        Parameters
        ----------
        dimension : `DimensionElement` or `str`
            The dimension for which fields should be returned.
        region : `bool`
