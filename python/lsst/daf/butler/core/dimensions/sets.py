...
class DimensionSet(Set):
    ...
    def __getitem__(self, key):
        try:
            return self._elements[key.name]
        except AttributeError:
    ...
    def get(self, key, default=None):
        ...
        """
        try:
            return self._elements.get(key.name, default)
        except AttributeError:
    ...
    def union(self, *others):
        ...
        *others : iterable over `DimensionElement` or `str`.
        ...
        -------
        result : `DimensionSet`
            A new set containing all elements in any input set.
        """
    ...
    def intersection(self, *others):
        ...
        *others : iterable over `DimensionElement` or `str`.
        ...
        -------
        result : `DimensionSet`
            A new set containing any elements in all input sets.
        """
        names = set(self.names)
        for other in others:
            names &= toNameSet(other).names
    ...
    def symmetric_difference(self, other):
        ...
        Parameters
        ----------
        other : iterable over `DimensionElement` or `str`.
...
class DimensionNameSet:
    ...
    def symmetric_difference(self, other):
        ...
        """
        names = self.names ^ toNameSet(other).names
        universe = getattr(other, "universe", None)
        if universe is not None:
            return DimensionSet(universe, names)
        else:
            return DimensionNameSet(names)
    ...
    # Operators that return sets are only enabled for DimensionNameSet
    # operands on both sides, to avoid confusion about return types.

    def __or__(self, other):
        if isinstance(other, DimensionNameSet):
            return self.union(other)
        return NotImplemented
