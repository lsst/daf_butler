...
__all__ = ["DatasetExport", "RepoExport",
...
class RepoExport:
    ...
    def saveDatasets(self, refs: Iterable[DatasetRef], *,
                     elements: Optional[Iterable[DimensionElement]] = None,
                     rewrite: Optional[Callable[[DatasetExport], DatasetExport]] = None):
        ...
        rewrite : callable, optional
            A callable that takes a single `DatasetExport` argument and returns
            a modified `DatasetExport`.  This is typically used to rewrite the
            path generated by the datastore.  If `None`, the `DatasetExport`
        ...
        """
        dataIds = set()
        datasets: Mapping[Tuple[DatasetType, Run], List[DatasetExport]] = defaultdict(list)
        for ref in refs:
...
class RepoExportBackend(ABC):
    ...
    @abstractmethod
    def saveDatasets(self, datasetType: DatasetType, run: Run, *datasets: DatasetExport,
        ...
        datasets : `DatasetExport`, variadic
