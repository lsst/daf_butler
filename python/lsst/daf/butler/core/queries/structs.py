...
class QuerySummary:
    ...
    def __init__(self, requested: DimensionGraph, *,
        ...
        self.dataId = dataId if dataId is not None else ExpandedDataCoordinate(self.universe.empty, ())
        ...
        self.expression = (expression if isinstance(expression, QueryWhereExpression)
                           else QueryWhereExpression(self.universe, expression))
    ...
    def spatial(self) -> NamedValueSet[DimensionElement]:
        ...
        result = self.requested.getSpatial(prefer=self.entire) - self.given.elements
        if len(result) == 1:
    ...
    def temporal(self) -> NamedValueSet[DimensionElement]:
        ...
        # - it isn't also given at query construction or execution time.
        result = self.requested.getTemporal(prefer=self.entire) - self.given.elements
    ...
    def mustHaveKeysJoined(self) -> DimensionGraph:
        """Dimensions whose primary keys must be included in the FROM clause
