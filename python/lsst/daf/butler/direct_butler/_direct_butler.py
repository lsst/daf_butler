...
class DirectButler(Butler):  # numpydoc ignore=PR02
    ...
    def _ingest_file_datasets(
        ...
        # Bulk insert.
        grouped_refs, dimension_records, _ = self._prepare_for_import_refs(
            self,
            refs,
            skip_missing=False,
        ...
        self._import_dimension_records(dimension_records, dry_run)
        imported_refs = self._import_grouped_refs(
            grouped_refs, self, progress, dry_run=dry_run, expand_refs=True
        )
    ...
    def _prepare_for_import_refs(
        ...
        defaultdict[_RefGroup, list[DatasetRef]],
        dict[DimensionElement, dict[DataCoordinate, DimensionRecord]],
        ...
        return grouped_refs, dimension_records, artifact_existence

    def _import_dimension_records(
        self, dimension_records: dict[DimensionElement, dict[DataCoordinate, DimensionRecord]], dry_run: bool
    ) -> None:
    ...
    def transfer_from(
        ...
        grouped_refs, dimension_records, artifact_existence = self._prepare_for_import_refs(
            source_butler,
            source_refs,
        ...
        with self.transaction():
            self._import_dimension_records(dimension_records, dry_run)
            imported_refs = self._import_grouped_refs(grouped_refs, source_butler, progress, dry_run=dry_run)
