...
from ..core import DataCoordinate, DataIdValue, DatasetRef, DatasetType
...
class CollectionSummary:
    ...
    def is_compatible_with(
        self,
        dataset_type: DatasetType,
        dimensions: Mapping[str, AbstractSet[DataIdValue] | EllipsisType | UnboundableSet[DataIdValue]],
        ...
        dimensions : `Mapping`
            ...
            to a type accepted by `UnboundableSet.coerce`.  If this is disjoint
            with the data IDs actually present in the collection for any of the
            dimensions of the dataset type, `False` will be returned.  Only
            governor dimensions are checked, but as a convenience other
            dimensions may be passed in as well.
        ...
        for gov_name in self.governors.keys() & dataset_type.dimensions.names & dimensions.keys():
            values_in_collection = UnboundableSet(self.governors[gov_name])
