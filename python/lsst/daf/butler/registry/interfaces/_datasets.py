...
class DatasetIdGenEnum(enum.Enum):
    ...
    DETERMINISTIC = 1
    """In deterministic mode ID is computed from a combination of dataset
    type, dataId, and run collection name.
    """
...
class DatasetRecordStorage(ABC):
    ...
    def insert(self, run: RunRecord, dataIds: Iterable[DataCoordinate],
        ...
        idMode : `DatasetIdGenEnum`
            ...
            With `DETERMINISTIC` then ID is computed from dataset type, dataId,
            and run collection name; if the same ID is already in the database
            then new record is not inserted.

        Returns
        -------
        datasets : `Iterable` [ `DatasetRef` ]
            References to the inserted (or existing in case of `DETERMINISTIC`)
            datasets.
        """
