...
| `TESTDIR` in the datastore files | `tests/test_datastore.py` defined it as `os.path.dirname(__file__)`; the split gave `test_datastore_file.py` and `test_datastore_constraints.py` the `os.path.abspath(...)` form instead. Normalized back to the original. `__file__` is absolute from Python 3.9, so the value never differed; the point is that the split must move code, not edit it. |
...
| --- | --- | --- |
| **`ButlerServerPostgresTests` never used postgres.** `setUpClass` started an instance into `cls.postgres`, and `ButlerServerTests.setUp` called `create_test_server(TESTDIR)` without passing it. `create_test_server`'s `postgres` argument defaults to `None`, so the server ran on sqlite; `cls.postgres` was written and never read. Verified directly: `create_test_server(TESTDIR)` yields a `SqliteDatabase` and `create_test_server(TESTDIR, postgres=pg)` a `PostgresqlDatabase`. | `tests/test_butler.py:3584` and `:3667` before this branch | Fixed here rather than deferred, because the fixture design passes the instance through and reproducing the bug would have meant writing it in deliberately. This is also why the coverage tool measured `server-postgres` as zero unique lines and zero unique arcs: it was executing the same sqlite path as `server-sqlite`. The axis was not redundant, it was broken, and DM-44842's regression could have recurred unseen. |
...
| `tests/test_butler.py::PosixDatastoreTransfers::testAbsoluteURITransferDirect` | `tests/test_butler_transfers.py::test_absolute_uri_transfer_direct[posix]` |
| `tests/test_butler.py::ChainedDatastoreTransfers::testAbsoluteURITransferDirect` | `tests/test_butler_transfers.py::test_absolute_uri_transfer_direct[chained]` |
| `tests/test_butler.py::PosixDatastoreTransfers::testAbsoluteURITransferUnsafeDirect` | `tests/test_butler_transfers.py::test_absolute_uri_transfer_unsafe_direct[posix]` |
| `tests/test_butler.py::ChainedDatastoreTransfers::testAbsoluteURITransferUnsafeDirect` | `tests/test_butler_transfers.py::test_absolute_uri_transfer_unsafe_direct[chained]` |
| `tests/test_butler.py::PosixDatastoreTransfers::testAbsoluteURITransferCopy` | `tests/test_butler_transfers.py::test_absolute_uri_transfer_copy[posix]` |
| `tests/test_butler.py::ChainedDatastoreTransfers::testAbsoluteURITransferCopy` | `tests/test_butler_transfers.py::test_absolute_uri_transfer_copy[chained]` |
