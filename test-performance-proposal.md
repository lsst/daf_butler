...
Re-measured after the review fixes, against `78e7f2904` checked out in a separate worktree, two interleaved repetitions each: **345.0s and 347.7s** of CPU on main against **255.3s and 269.9s** on the branch, saving 89.7s and 77.8s, or 26.0% and 22.4%.
Take the range rather than either figure: the two branch repetitions differ by 5.7% where the two main repetitions differ by 0.8%, which is the run-to-run noise this machine produces even in CPU time.
The branch is the cheaper side while also running the 12 tests of the new cache module that main does not have.

Seven changes landed on `tickets/DM-55885`, each independently revertable:

| Commit | Change | Effect |
| --- | --- | --- |
| `e82ae5941` | Template cache for test repository creation | repository setup 151.0s to 87.8s |
| `f7aad3a6d` | Only cache repositories with a SQLite registry | template builds 171 to 48 over the PostgreSQL modules |
| `1e51ced09` | Split `Butler.makeRepo` into two phases | behaviour-neutral |
| `196a57642` | Cache configuration and database separately | about 1s |
| `b16f67fd9` | `Config` merge dict fast path | merge 0.192ms to 0.084ms |
| `27713369c` | Dedicated copy routine in `Config.__init__` | copy 111us to 48us |
| `77a7eff80` | Reuse storage class definitions from an identical config | 18s, `addFromConfig` 8.80ms to 0.37ms |

Adoption of the helper across the test modules sits in six further commits between `e82ae5941` and `f7aad3a6d`.
Review fixes are squashed into the commits above, so these hashes date from that rebase; only `e82ae5941` and the three adoption commits immediately after it survive from the original sequence.
A hash cited here should be checked with `git merge-base --is-ancestor`, not `git log -1`, because a rewritten commit stays resolvable through the reflog long after it leaves the branch.
...
Several details each caused a real failure, in development or in review.
...
The returned `Config` must describe the caller's repository rather than the template's, and it is produced by copying the `Config` that creation returned and repointing `configFile` at the new file.
Re-reading the copied `butler.yaml` instead looks equivalent and is not.
`_make_repo_butler_config` strips `registry.managers.obscore.config` before writing, because that configuration is stored in the registry rather than the file, while still returning it in memory for registry creation to use.
A copy that re-read the file therefore initialised the registry without its obscore configuration, silently and only for repositories that reached the cache twice.
`test_obscore_config_survives_a_cache_hit` covers it.

The helper must not be more permissive than the method it stands in for.
`overwrite=True` bypasses the cache, so the copy path always means `overwrite=False`; it creates `butler.yaml` exclusively rather than overwriting it, and an occupied root raises `FileExistsError` exactly as writing the configuration would.
...
The same rule binds this package's own tests, and three of them had to be moved back.
`tests/test_butler.py::testMakeRepo` asserts on what repository creation writes, and the two `TestMissingObscoreConfig` tests in `tests/test_obscore.py` assert on a warning emitted only while the configuration is written.
They call `Butler.makeRepo` directly.
Each passed through the helper only because its configuration embedded a per-test temporary path and so never hit the cache, which is accident rather than isolation.

### What a cache key has to identify

A key must identify the *contents* that reach the repository, never the name of a file holding them.

A configuration supplied as a path is read into a `Config` on entry, so a test that rewrites a temporary configuration file in place is not served the earlier version; this also removes a duplicate read the cacheability check was performing.
A dimension configuration supplied as a path is expanded through `DimensionConfig`, which is what resolves a relative name against the configuration search path.

Two inputs outside the configurations themselves belong in the keys.
`forceConfigRoot` decides whether root-dependent values in the supplied configuration survive into the written file.
`DAF_BUTLER_CONFIG_PATH` decides which default files a configuration is expanded against, and `DimensionConfig(None)` reads its defaults from it, so it keys the database as well as `butler.yaml`.
Only a handful of tests vary any of these, so including them costs a miss in those tests and nothing elsewhere.
...
Sharing instances between uses is safe, and review asked why.
A `StorageClass` describes a definition rather than holding state: its only mutation is memoisation of the imported Python type and removal of the converters whose types cannot be imported, both determined by the definition itself.
A shared instance therefore cannot carry information from one use to another.
This is not a new property either, because `StorageClassFactory` is a singleton and already shared one instance per definition across every use before this change; what the cache adds is that `reset()` re-registers those instances instead of rebuilding equal ones.
...
A fresh `git worktree` cannot run this suite: it lacks the gitignored generated `version.py`, so all 63 test modules fail to import, and it lacks the equally gitignored `bin/butler`.
`scons version bin` supplies both, and neither is a side effect of `scons python`, which reports `Nothing to be done` in a tree missing both.
...
A comparison run has to be symmetric in these generated artefacts as well as in the code.
Without `bin/butler` the two `tests/test_cliLog.py` subprocess tests fail rather than run, so a tree that has it executes two tests that a tree without it does not.
...
`tests/test_server.py::ButlerClientServerAuthorizationTestCase::test_group_authorization` fails on this branch with an unmodified working tree, and fails identically at `78e7f2904`.
A verification run has to expect it, or a real regression hides beside it.

## Corrections to earlier estimates in this document's history

Three claims here were wrong and are recorded so the reasoning is not repeated.
...
The instruction to re-read the returned `Config` from the copied file was wrong, and is corrected above.
It came from a real failure, since returning the template's own `Config` gave callers the template's root, but the fix chosen treated the written file as equivalent to the returned configuration.
The written file is deliberately not equivalent, and the difference is silent rather than an error.
The general lesson is that a cache which reconstructs its value from a side effect of the operation it replaces has to prove that side effect is lossless.
